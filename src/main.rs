...
use crate::cli::{Cli, Commands, ConfigAction, OutputFormat};
...
async fn run(cli: Cli) -> anyhow::Result<()> {
    match cli.command {
        Commands::Agenda { format, limit, days } => {
            ...
            let events = client.get_events(days_ahead, event_limit).await
        ...
        Commands::ListCalendars => {
            ...
            for calendar in calendars {
                ...
                println!("  {}: {}{}", id.bright_green(), name, primary.bright_yellow());
            }
        },
        Commands::Config { action } => {
            match action {
                ...
                ConfigAction::Init => {
                    ...
                    println!("1. Configuration: {}", Config::load()?.expand_path("~/.config/callux/config.toml").bright_yellow());
                    println!("2. Credentials: {}", config.expand_path(&config.auth.credentials_path).bright_yellow());
